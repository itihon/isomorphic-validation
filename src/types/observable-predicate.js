...
export default function ObservablePredicate(
    ...
    */
    ...
    function obsPredicate(forbidInvalid = keepValid, id) {
        ...
        var result = fn(...items.map(item => item.getValue()));
        
        if (result.then) {
            return previousMicrotask = result.then(
                res => {
    ...
    return  Object.defineProperties(
        obsPredicate,
        {
            ...
            invalidate: { value: () => setValidity(notifySubscribers(false), validationResult) },
